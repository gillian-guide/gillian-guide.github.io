...
| Глубина поля наблюдения | Откл | Откл | Эта настройка регулирует глубину резкости и размытие в движении - однако, она также вызывает проблемы с размытой картинкой на ПК и эффектами, которые просто не масштабируются по разрешению - используйте [FusionFix](/Essential-Modding/FusionFix/) или [Shader Fixes](/Essential-Modding/Shader-Fixes/) для исправления проблемы. Можно быстро переключать кнопкой ++p++. |
