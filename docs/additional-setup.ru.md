...
| Качество воды | Среднее | Среднее | Эта настройка регулирует плотность и интенсивность волн в воде и образцах воды, а также разрешение отражений в воде. Рекомендуется использовать Среднее, так как оно менее экстремально и более реалистично. [FusionFix](essential-modding/fusionfix.md) позволяет сохранить максимальное разрешение отражений при более низких параметрах. |
...
| Shadow Filter | PCSS или Softest | ? | Эта настройка смягчает тени и приближает их к реальному поведению (при использовании PCSS). <br>==Доступно только при использовании [FusionFix](essential-modding/fusionfix.md).==</br> |
...
| FPS Limiter | 60 | 30 | Рекомендуется установить значение на 60, чтобы избежать [проблем с таймингом](https://www.pcgamingwiki.com/wiki/Grand_Theft_Auto_IV#Timing-related_issues). <br>==Доступно только при использовании [FusionFix](essential-modding/fusionfix.md).==</br> |
| Motion Blur | Вкл | Вкл | Эта настройка управляет эффектом размытия в движении. <br>==Доступно только в меню `Настройки` - `Экран` при использовании [FusionFix](essential-modding/fusionfix.md).==</br> |
...
| Distant Blur | По предпочтению | Very High (?) | Эта настройка управляет уровнем размытия вдали от настройки Depth of Field. <br>==Доступно только в меню `Настройки` - `Экран` при использовании [FusionFix](essential-modding/fusionfix.md).==</br> |
| Depth of Field | Вкл | Вкл | Эта настройка управляет глубиной резкости. <br>==Доступно только в меню `Настройки` - `Экран` при использовании [FusionFix](essential-modding/fusionfix.md).==</br> |
| TreeFX | PC или PC+ | Console | Этот параметр управляет поведением окклюзии растительности. <br>==Доступно только в меню `Настройки` - `Экран` при использовании [FusionFix](essential-modding/fusionfix.md).==</br> |
