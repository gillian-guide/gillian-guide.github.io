...
| Глубина поля наблюдения/Motion Blur | Откл | Откл | Эта настройка переключает глубину резкости* и размытие в движении - однако, она также вызывает проблемы с размытой картинкой на ПК и эффектами, которые просто не масштабируются по разрешению - используйте [FusionFix](essential-modding/fusionfix.md) и [Shader Fixes](essential-modding/shader-fixes.md) для исправления проблем - однако при этом настройка изменяется для переключения только размытия в движении. (см. следующую настройку для переключения глубины резкости). На текущий момент, опции Откл и Вкл идут наоборот - это будет исправлено в будущих обновлениях [Shader Fixes](essential-modding/shader-fixes.md). Можно быстро переключать кнопкой ++p++. |
