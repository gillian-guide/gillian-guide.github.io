...
| Definition/Motion Blur | Off | Off | This setting controls depth of field and motion blur - however, it is also known to cause problems with blurred image on PC and effects that simply do not scale up in resolution - use [FusionFix](essential-modding/fusionfix.md) and [Shader Fixes](essential-modding/shader-fixes.md) to solve the problems - however, this changes the setting to toggle Motion Blur only. (see the next setting for toggling depth of field). At the time, Off and On options are reversed - this will be changed in future [Shader Fixes](essential-modding/shader-fixes.md) updates. Can be quickly toggled by pressing ++p++ button. |
