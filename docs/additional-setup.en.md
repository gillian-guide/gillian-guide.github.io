...
| Definition | On | Off | ==Ignore this and see the next two options if you're using [FusionFix](essential-modding/fusionfix.md) and [Shader Fixes](essential-modding/shader-fixes.md).==<br>This setting controls depth of field and motion blur - however, it is also known to cause problems with blurred image on PC and effects that simply do not scale up in resolution - due to that, either keep this option On and lose the effects or install [FusionFix](essential-modding/fusionfix.md) and [Shader Fixes](essential-modding/shader-fixes.md) and see the next two options instead. Can be quickly toggled by pressing ++p++ button.</br> |
| Motion Blur | Off | Off | This setting toggles motion blur. At the time, Off and On options are reversed - this will be changed in future [Shader Fixes](essential-modding/shader-fixes.md) updates. Can be quickly toggled by pressing ++p++ button. ==Only appears when using [FusionFix](essential-modding/fusionfix.md). The change itself comes from [Shader Fixes](essential-modding/shader-fixes.md).== |
