...
| Motion Blur | On | On | This setting toggles motion blur. Can be quickly toggled by pressing ++p++ button. <br>==Only available when using [FusionFix](essential-modding/fusionfix.md).==</br> |
| Depth of Field | On | On | This setting controls depth of field. <br>==Only available when using [FusionFix](essential-modding/fusionfix.md).==</br> |
...
| Console Gamma | On | On | [Changes the gamma to match the console-like look.](essential-modding/assets/console-gamma.png) <br>==Only appears in `Settings` - `Display` when using [FusionFix](essential-modding/fusionfix.md).==</br> |
| Screen Filter | Default | Default | Allows you to switch the timecyc file to match different screen filters (e.g. to have TBoGT filters in IV and vice versa). <br>==Only appears in `Settings` - `Display` when using [FusionFix](essential-modding/fusionfix.md).==</br> |
