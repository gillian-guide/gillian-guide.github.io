...
=== "Ванилла"
    ...
    | Качество фильтрации текстур | Анизотропная x16 | Эта настройка регулирует фильтрацией текстур. |
...
=== "FusionFix"
    ...
    | Транспортный поток | До 70<br>100, если есть кастомный popcycle (архив) | Эта настройка регулирует количество транспорта по дорогах. <br>**Установка слишком высокого значения без кастомного popcycle делает езду по дороге очень неприятной, особенно с нестабильным ИИ траффика, который иногда застревает даже на прямых дорогах и мостах.** |
    ...
    | Тональное отображение | Вкл | Эта настройка регулирует кастомным тональным отображением, которое исправляет "пересвет" (слишком яркие объекты, которые игра не может отобразить в полной мере). |
    | Окклюзия окружающей среды | Вкл | Этот параметр управляет кастомным эффектом окклюзии окружающей среды, который создает тени на тех частях объектов, которые пересекаются с другими объектами (например, между стенами и землей). Возможен удар по производительности.  <br>Настройка поглубже доступна в :material-file-cog:`GTAIV.EFLC.FusionFix.ini`. |
    | Фильтрация теней | CHSS<br>По предпочтению | Эта настройка регулирует резкость теней (так как в реальной жизни тени довольно мягкие). <br>Настройка поглубже доступна в :material-file-cog:`GTAIV.EFLC.FusionFix.ini`. |
    ...
    | Автоэкспозиция консоли | По предпочтению | Эта настройка регулирует, будет ли применяться более жесткая автоэкспозиция консоли (эффект «ослепления» при выходе из помещения). |
    ...
    | Автоматически ставить игру на паузу в фоне | Откл | Эта настройка регулирует, будет ли процесс игры "останавливаться" когда окно игры теряет фокусировку (альт-таббинг).<br>Рекомендуется ставить на **Откл** во избежание потенциальных вылетов. |
    | Дополнительные ночные тени | По предпочтению<br>Откл | Эта настройка определяет, будут ли отображаться тени от фонарей и фар. Она работает крайне нестабильно, но если вам не хватает эффекта из базовой версии IV, вы можете включить его. |

=== "Steam Deck"
    !!! info "Что это за настройки?"
        Эти настройки адаптированы под Steam Deck и его аналоги в виде портативных ПК, с целью 800р 40-60 FPS (проверено на Legion Go с SteamOS используя режим Balanced TDP благодаря Not AJ)

    | Настройка | Оптимальное значение | Описание |
    | :-------: | :------------------: | :------: |
    | Видеорежим | 1280x800 | Эта настройка регулирует разрешение вашего монитора или, если включена настройка `Оконный режим`, размер игрового окна. |
    | Соотношение сторон | Авто | Эта настройка регулирет соотношение сторон экрана относительно разрешения вашего монитора. |
    | Качество текстур | Высокое | Эта настройка регулирует разрешение всех текстур. |
    | Разрешение отражений | Высокое | Эта настройка регулирует разрешение отражений (кроме отражений в воде). |
    | Качество воды | Высокое | Эта настройка регулирует *только* разрешение отражений воды когда используется FusionFix. |
    | Качество теней | Высокое | Эта настройка регулирует разрешение и расстояние рендеринга теней.<br>**Понизьте до *Среднее* для небольшого прироста FPS.**<br>Настройка поглубже доступна в :material-file-cog:`GTAIV.EFLC.FusionFix.ini`. |
    | Улучшенные ночные тени | Высокое | Эта настройка регулирует сколько элементов локального света (например, фар автомобиля) могут отбрасывать тени, а также само разрешение этих теней. **Отключите для небольшого прироста FPS.** <br>Настройка поглубже доступна в :material-file-cog:`GTAIV.EFLC.FusionFix.ini`. |
    | Качество фильтрации текстур | Анизотропная x4 | Эта настройка регулирует фильтрацией текстур. |
    | Дистанция обзора | 25 | Эта настройка регулирует основное расстояние рендеринга LOD для таких объектов, как здания и транспорт. Также влияет на расстояние рендеринга пропов. <br>**Установка значения выше *70* не имеет сущных улучшений.** |
    | Глубина детализации | 31 | Эта настройка регулирует расстояние вторичного рендеринга LOD для деталей в пропах.<br>**Установка значения выше *70* не имеет сущных улучшений.** |
    | Транспортный поток | 50<br>100, если есть кастомный popcycle (архив) | Эта настройка регулирует количество транспорта по дорогах. <br>**Установка слишком высокого значения без кастомного popcycle делает езду по дороге очень неприятной, особенно с нестабильным ИИ траффика, который иногда застревает даже на прямых дорогах и мостах.** |
    | Вертикальная синхронизация | Откл | Эта настройка регулирует вертикальную синхронизацию. |
    | Ограничитель кадров | 60<br>30 | Эта настройка регулирует ограничением кадров.<br>**Рекомдентуеся установить значение на 60 или 30 во избежание [проблем с таймингом](https://github.com/GTAmodding/GTAIV-Issues-List/issues/112), хотя сюжет может быть пройден и выше 60, если игнорировать баги. **<br>[Lossless Scaling](https://store.steampowered.com/app/993090/Lossless_Scaling/) может быть использован для искусственного увеличения частоты кадров.<br>Настройка поглубже доступна в :material-file-cog:`GTAIV.EFLC.FusionFix.ini`. |
    | Сглаживание | SMAA | Эта настройка регулирует сглаживание. |
    | Объёмный туман | Вкл<br>По предпочтению | Эта настройка регулирует кастомный объёмный туман, который появляется на расстоянии, в определнные типы погод или при подьеме на высоту. **Отключите для небольшого прироста FPS**. |
    | Солнечные лучи | Вкл<br>По предпочтению | Эта настройка регулирует кастомные лучи солнца.<br>Настройка поглубже доступна в :material-file-cog:`GTAIV.EFLC.FusionFix.ini`. |
    | Увеличенная дистанция солнечного света | Выкл<br>По предпочтению | Эта настройка регулирует возможность света от солнца достигать мест, ранее недостежимых. **Возможен удар по производительности.** |
    | Тональное отображение | Вкл | Эта настройка регулирует кастомным тональным отображением, которое исправляет "пересвет" (слишком яркие объекты, которые игра не может отобразить в полной мере). |
    | Окклюзия окружающей среды | Выкл<br>По предпочтению | Этот параметр управляет кастомным эффектом окклюзии окружающей среды, который создает тени на тех частях объектов, которые пересекаются с другими объектами (например, между стенами и землей). **Возможен удар по производительности.**  <br>Настройка поглубже доступна в :material-file-cog:`GTAIV.EFLC.FusionFix.ini`. |
    | Фильтрация теней | Sharp<br>По предпочтению | Эта настройка регулирует резкость теней (так как в реальной жизни тени довольно мягкие). <br>Настройка поглубже доступна в :material-file-cog:`GTAIV.EFLC.FusionFix.ini`. |
    | Графический API | DirectX 9 | Эта настройка управляет используемым графическим API. Немного не интуитивно, но желательно дать системе самостоятельно утилизировать DXVK, по этому оставьте настройку на DirectX 9 (на Windows используйте Vulkan). |
    ...
    | :-------: | :------------------: | :------: |
    | Оконный режим | Вкл | Эта настройка регулирует, находится ли игра в экслюзивном полноэкранном режиме или (безрамночном) оконном.<br>Безрамочный режим в основном более рекомендован, чем экслюзивный полноэкранный в пользу мультизадачности.  |
    | Оконный режим без рамки | Вкл | Эта настройка регулирует, находится ли игра в обычном окне или безрамочном когда **включена** настройка `Оконный`. |
    | Автоматически ставить игру на паузу в фоне | Откл | Эта настройка регулирует, будет ли процесс игры "останавливаться" когда окно игры теряет фокусировку (альт-таббинг).<br>Рекомендуется ставить на **Откл** во избежание потенциальных вылетов. |
    | Дополнительные ночные тени | По предпочтению<br>Откл | Эта настройка определяет, будут ли отображаться тени от фонарей и фар. Она работает крайне нестабильно, но если вам не хватает эффекта из базовой версии IV, вы можете включить его. |
...
=== "Идентичные к консоли"
    ...
    | Окклюзия окружающей среды | Откл | Этот параметр управляет кастомным эффектом окклюзии окружающей среды, который создает тени на тех частях объектов, которые пересекаются с другими объектами (например, между стенами и землей). Возможен удар по производительности. |
    ...
    | Настройка | Оптимальное значение | Описание |
    | :-------: | :------------------: | :------: |
    | Дополнительные ночные тени | Откл | Эта настройка определяет, будут ли отображаться тени от фонарей и фар. |
